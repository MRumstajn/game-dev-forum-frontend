export const TOP_CATEGORIES_STATISTIC_URL = "/statistics/popular-categories";

export const CATEGORY_URL = "/categories";

export const CATEGORY_STATISTICS_URL = "/statistics/category-statistics";
export const OVERALL_STATISTICS_URL = "/statistics/overall-statistics";

export const THREAD_URL = "/threads";

export const THREAD_SEARCH_URL = "/threads/search";

export const THREAD_STATISTICS_URL = "/statistics/thread-statistics";

export const THREAD_WITH_LATEST_ACTIVITY_URL =
  "/threads/search/latest-activity";

export const CATEGORY_SEARCH_URL = "/categories/search";

export const POST_URL = "/posts";

export const POST_SEARCH_URL = "/posts/search";

export const POST_WITH_LATEST_ACTIVITY_URL = "/posts/search/latest-activity";

export const LOGIN_URL = "/auth/login";

export const HOME_URL = "/home";

export const SECTION_SEARCH_URL = "/sections/search";

export const CURRENT_USER_URL = "/auth/current-user";

export const POST_REACTION_COUNT_URL = "/post-reactions/counts";

export const POST_REACTION_URL = "/post-reactions";

export const POST_REACTION_SEARCH_URL = "/post-reactions/search";

export const USER_URL = "/users";

export const USER_CHANGE_PASSWORD_URL = "/auth/change-password";
